...
namespace Analog::Filters
{
    struct RCFilter : public FilterProcessor
    {
        ...
        DspFloatType          out;
        DspFloatType          freq=440.0,sampleRate=44100;
        enum {
            LOW,
            HIGH,
            BAND,
        ...
        RCFilter(int max, DspFloatType amplify, DspFloatType sr=44100) : FilterProcessor()
        {
            sampleRate = sr;
            this->max = max;
        ...
        void RC_bandpass(DspFloatType sound)
        {
            ...
            for (a = 0; a < max; a++) {
                RC_filter(sound, HIGH, a);
                RC_filter(sound, LOW, a);
            }
        }
