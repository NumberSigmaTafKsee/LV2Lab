...
namespace FX
{
    ...
    // AutoWah
    ///////////////////////////////////////////////////////////////////////////////
    inline Filters::FilterCoefficients makeWahWahFilter(DspFloatType sampleRate,DspFloatType centreFrequency, DspFloatType Q, DspFloatType gain)
    {
        if( Q <= 0) Q = 0.001;
        ...
        const DspFloatType discreteFrequency = 2.0 * M_PI * centreFrequency/sampleRate;
    ...
    struct AutoWah : public StereoFXProcessor
    {
        ...
        {
            Random noise;
            baseFrequency_ = 10.0*noise.frand();
            q_ = 10.0*noise.frand() + 0.5;
            lfoFrequency_   = 10.0*noise.frand();
            lfoWidth_       = 2000.0*noise.frand();   
            envelopeWidth_  = noise.frand();
            envelopeAttack_ = noise.frand();
            envelopeDecay_  = noise.frand();
        ...
        AutoWah(DspFloatType sr=44100.0) : StereoFXProcessor()
        {
            sampleRate = sr;
        ...
        void ProcessBlock (size_t numSamples, DspFloatType ** inputs, DspFloatType ** outputs)
        {
            ...
            for(channel = 0; channel < std::min(numInputChannels, numWahFilters_); ++channel)
            {
                ...
                for (int sample = 0; sample < numSamples; ++sample)
                {
                    ...
                    Filters::FilterCoefficients c = makeWahWahFilter(
                                                            sampleRate,
                                                            centreFrequency,
                                                            q_,
                                                            0.5);
