...
namespace FX
{
    ...
    inline Filters::FilterCoefficients makeTwoPoleAllpass ( DspFloatType sampleRate, DspFloatType centreFrequency,
                                                                DspFloatType Q) 
    {
        if(Q <= 0) Q = 0.001;
    ...
    struct Phaser : public StereoFXProcessor
    {
        ...
        Phaser(DspFloatType sr = 44100.0) : StereoFXProcessor() {
            sampleRate = sr;
