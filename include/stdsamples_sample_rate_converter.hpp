...
namespace Polyphase
{
    ...
    float* src_hamming(int num_taps) {
        float* window = (float*)malloc(num_taps * sizeof(float));
    ...
    float* src_fir_prototype(int num_taps, float cutoff_low, float cutoff_high, 
        ...
        float* proto = (float*)malloc(num_taps * sizeof(float));
    ...
    void src_taps_to_pfb(float* coefficients, int num_taps, int interpolation, 
        ...
        float* pfb = (float*)malloc(pfb_size * sizeof(float));
        int c_index = 0;
    ...
    FIR_Filter* src_generate_fir_filter(float* coefficients, int num_taps,
            int interpolation, int decimation) {
        FIR_Filter* filter = (FIR_Filter*)malloc(sizeof(FIR_Filter));
        ...
        filter->history = (float*)malloc(filter->history_length * sizeof(float));
