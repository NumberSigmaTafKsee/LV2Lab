...
struct Plot
{
    ...
    void data_style(const char * style)
    {
        cmd("set style data \"%s\"",style);
    }
    void boxwidth(double w,const char *s = "")
    {
        cmd("set boxwidth \"%f\" \"%s\"",w,s);
    }
    void set_fill_solid(double w)
    {
        cmd("set style fill solid \"%f\"",w);
    }
    void clear() {
        cmd("clear");
    }
    ...
    void key(bool value)
    {
        if(value) cmd("set key on");
        else cmd("set key off");
    }
    void xrange(int start, int end)
    {
        cmd("xrange[\"%d\":\"%d\"]",start,end);
    }
    void yrange(int start, int end)
    {
        cmd("yrange[\"%d\":\"%d\"]",start,end);
    }
    void ticks(int inc)
    {
        cmd("set xtic \"%d\" nomirror",inc);
    }
    void unset_yticks() {
        cmd("unset yics");
    }
    ...
    {
        char const *    cmdc    = (handle->nplots > 0) ? "replot" : "plot";
        title                  = (title == NULL)      ? "(none)" : title;

        cmd("%s %.18e * x + %.18e title \"%s\" with %s",
                    cmdc, a, b, title, handle->pstyle) ;
