...
namespace Noise::NoiseGenerators
{
  ...
  public:
    ...
    float tick() {
      ...
      Random noise;
    // unrolled loop
      float temp = float( noise.frand() );
      state[0] = P[0] * (state[0] - temp) + temp;
      temp = float( noise.frand() );
  ...
  {
      Random noise;
      float R1 = noise.frand();
      float R2 = noise.frand();
  ...
  inline float GaussRand (int m, float s)
  {
    ...
    if (pass)
    {
        ...
        do {
          x1 = 2.0f * noise.frand () - 1.0f;
          x2 = 2.0f * noise.frand () - 1.0f;
          w = x1 * x1 + x2 * x2;
        } while (w >= 1.0f);
