...
namespace kfr3
{
	// a delayline is really a fir filter
	// it can also have an IIR
	// a comb filter is really a delay
    ...
    struct MultiTapDelayLine 
    {
        ...
        {       
            ...
            return mix*I + (1.0-mix)*(mix*(sum+output));            
        }
    ...
    struct MultiTapDelayLineFIR
    {
        ...
        MultiTapDelayLineFIR(T delayTime=0.5)
        {            
            delay.resize(sampleRate);            
            zeros(delay);
            feedback=0.5;
        ...
        {       
            ...
            T * dly = delay.data();
            T * forward = fir.data();
            #pragma omp simd aligned(dly,forward)
            for(size_t i = 0; i < fir.size(); i++)
            {                            
				T x1 = 0;		
				int rx = r;		
				delay.ringbuf_read(r,x1);  
				output = x1;
				T f  = output - (int)output;                                                                
				output = Interpolate(rx,f);
				r = r % delayLen;
                sum += dly[i] * forward[i];
            }                                                   
            delay.ringbuf_write(write_cursor, I + Y*feedback*output);        
    ...
    template<typename T>
    struct MultiTapDelayLineIIR
    {
        size_t write_cursor, read_cursor;
        size_t delayLen;
        kfr::univector<T> delay;        
        kfr::univector<T> fir,iir;
        T feedback;
        T mix;
        ...
        MultiTapDelayLineIIR(T delayTime=0.5)
        {            
            delay.resize(sampleRate);            
            zeros(delay);
            feedback=0.5;
        ...
        void addFirTap(size_t i, T t) {
            fir[fir.size()-i-1] = t;
        }        
        void addIirTap(size_t i, T t) {
            iir[iir.size()-i-1] = t;
        }        
        T Tick(T I, T A=1, T X=1, T Y=1) 
        {       
            ...
            r = read_cursor;   
            #pragma omp simd aligned(dly,feedback)
