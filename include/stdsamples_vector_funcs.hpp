...
namespace AudioDSP
{
        ...
        size_t post, size_t pre, size_t pos)
    {
        ...
        if (pos < post + 1) {
            ...
            #pragma omp simd aligned(vec,medar)
            ...
            #pragma omp simd aligned(medar,vec)
            for (k = 0; k < length - pos + post; k++)
				medar[k] = vec[k + pos - post];
            #pragma omp simd aligned(medar)
    ...
    void adapt_thres(const size_t n, T * vec, T * tmp,
        ...
        size_t length = n, j;  
        #pragma omp simd aligned(vec,tmp)
        for (j=0;j<length;j++) {
    ...
    void autocorr (const size_t n, const T * input, T * output)
    {
        ...
        data = input;
        acf = output;
        #pragma omp simd aligned(data,acf)
        for (i = 0; i < length; i++) {
            tmp = 0.;
            for (j = i; j < length; j++) {
                tmp += data[j - i] * data[j];
