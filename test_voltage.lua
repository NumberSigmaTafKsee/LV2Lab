...
vco = voltage.VCO(44100,voltage.VCO.SAWTOOTH)
vcf = voltage.VCF(44100,1,0.5)
vca = voltage.VCA(1.25)
...
function noise(input,output,frames)        
    ...
    vco:ProcessBlock(frames,output)     
    --diode:ProcessBlock(frames,output,output)
    vcf:ProcessBlock(frames,output,output)
    --vca:ProcessBlock(frames,output,output)        
